...
public static partial class CatalogEndpoints
{
    ...
    {
        var form=await context.Request.ReadFormAsync(context.RequestAborted);var model=new ProductEditModel{Id=Int(form,"Id"),Title=Text(form,"Title"),Slug=Text(form,"Slug"),Code=Text(form,"Code"),ShortDescription=Text(form,"ShortDescription"),Description=Text(form,"Description"),ProductTypeId=Int(form,"ProductTypeId"),PrimaryCategoryId=NullableInt(form,"PrimaryCategoryId"),Status=Text(form,"Status"),IsFeatured=Checked(form,"IsFeatured"),RowVersion=Text(form,"RowVersion")};
    ...
    {
        var form=await context.Request.ReadFormAsync(context.RequestAborted);var productId=Int(form,"ProductId");var variantId=Int(form,"VariantId");var weight=Decimal(form,"ExactGoldWeightGrams");var tracking=Text(form,"TrackingCode");var status=Text(form,"Status");if(productId<=0||variantId<=0||weight<=0||string.IsNullOrWhiteSpace(tracking)||status is not("available" or "reserved" or "sold" or "damaged"))return Results.LocalRedirect($"/admin/catalog/products/edit/{productId}?step=4&error=piece-invalid");
        var result=await service.SavePieceAsync(variantId,Int(form,"Id"),tracking,weight,NullableDecimal(form,"StoneWeightCarats"),status,Checked(form,"IsActive"),Text(form,"RowVersion"),await ActorAsync(context,users),context.RequestAborted);return Results.LocalRedirect($"/admin/catalog/products/edit/{productId}?step=4{ResultQuery(result,"piece-saved",true)}");
    }

    private static async Task<IResult> SaveCategoryAsync(HttpContext context,UserManager<ApplicationUser> users,CatalogService service){var f=await context.Request.ReadFormAsync(context.RequestAborted);var id=Int(f,"Id");var name=Text(f,"Name");var slug=Text(f,"Slug");var parent=NullableInt(f,"ParentId");if(string.IsNullOrWhiteSpace(name)||name.Length>160||!ValidSlug(slug)||id==parent)return Redirect("/admin/catalog/categories","invalid");var r=await service.SaveCategoryAsync(id,name,slug,Text(f,"Description"),parent,Math.Max(0,Int(f,"DisplayOrder")),Checked(f,"IsActive"),Text(f,"RowVersion"),await ActorAsync(context,users),context.RequestAborted);return Results.LocalRedirect($"/admin/catalog/categories{ResultQuery(r,"saved")}");}
    ...
    private static bool IsAllowedImage(byte[] b,string type)=>b.Length>=12&&type is "image/jpeg" or "image/png" or "image/webp"&&((b[0]==0xFF&&b[1]==0xD8)||(b[0]==0x89&&b[1]==0x50&&b[2]==0x4E&&b[3]==0x47)||(b[0]==0x52&&b[1]==0x49&&b[8]==0x57&&b[9]==0x45));
    private static IResult Redirect(string path,string error,bool hasQuery=false)=>Results.LocalRedirect($"{path}{(hasQuery?'&':'?')}error={error}"); private static string ResultQuery(CatalogSaveResult r,string success,bool hasQuery=false)=>r==CatalogSaveResult.Saved?$"{(hasQuery?'&':'?')}status={success}":Error(r,hasQuery); private static string Error(CatalogSaveResult r,bool hasQuery=false)=>$"{(hasQuery?'&':'?')}error={(r==CatalogSaveResult.Conflict?"conflict":r==CatalogSaveResult.NotFound?"not-found":r==CatalogSaveResult.InUse?"in-use":r==CatalogSaveResult.Protected?"protected":"invalid")}";
}

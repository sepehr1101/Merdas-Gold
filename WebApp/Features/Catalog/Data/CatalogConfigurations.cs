...
public sealed class ProductCategoryConfiguration : IEntityTypeConfiguration<ProductCategory>
{
    ...
    {
        b.ToTable("ProductCategory"); b.HasKey(x => x.Id); b.Property(x => x.Name).HasMaxLength(160).IsRequired(); b.Property(x => x.Slug).HasMaxLength(180).IsRequired(); b.Property(x => x.Description).HasMaxLength(1000); b.Property(x => x.ImageUrl).HasMaxLength(300); b.Property(x => x.ImageContentType).HasMaxLength(30); b.Property(x => x.RowVersion).IsRowVersion();
        b.HasIndex(x => x.Slug).IsUnique(); b.HasOne(x => x.Parent).WithMany().HasForeignKey(x => x.ParentId).OnDelete(DeleteBehavior.Restrict);
        b.HasData(Category(1, "زیورآلات", "jewelry", null, 1), Category(2, "حلقه", "rings", 1, 5, "/assets/storefront/images/Untitled-4-min.png"), Category(3, "گردنبند", "necklaces", 1, 3, "/assets/storefront/images/Untitled-44-min.png"), Category(4, "دستبند", "bracelets", 1, 2, "/assets/storefront/images/Untitled72-4-min.png"), Category(5, "گوشواره", "earrings", 1, 6, "/assets/storefront/images/421-min.png"), Category(6, "آویز", "pendants", 1, 4, "/assets/storefront/images/Untitled-q4-min.png"), Category(7, "سرویس و نیم‌ست", "sets", 1, 7), Category(8, "سکه و شمش", "coins-bars", null, 2), Category(1009, "پابند", "anklets", 1, 1, "/assets/storefront/images/Untitl555ed-4-min.png"));
    }
    private static object Category(int id, string name, string slug, int? parentId, int order, string? imageUrl = null) => new { Id = id, Name = name, Slug = slug, Description = "", ParentId = parentId, DisplayOrder = order, IsActive = true, ShowOnHome = imageUrl is not null, ImageUrl = imageUrl };
}
...
{
    public void Configure(EntityTypeBuilder<Product> b) { b.ToTable("Product",t=>{t.HasCheckConstraint("CK_Product_Status","[Status] IN (N'draft',N'active',N'archived')");t.HasCheckConstraint("CK_Product_MakingFeePercent","[MakingFeePercent] IS NULL OR [MakingFeePercent] BETWEEN 0 AND 100");t.HasCheckConstraint("CK_Product_SellerProfitPercent","[SellerProfitPercent] IS NULL OR [SellerProfitPercent] BETWEEN 0 AND 100");}); b.HasKey(x=>x.Id); b.Property(x=>x.Title).HasMaxLength(250).IsRequired(); b.Property(x=>x.Slug).HasMaxLength(280).IsRequired(); b.Property(x=>x.Code).HasMaxLength(80).IsRequired(); b.Property(x=>x.ShortDescription).HasMaxLength(700); b.Property(x=>x.Description).HasColumnType("nvarchar(max)"); b.Property(x=>x.MakingFeePercent).HasPrecision(9,4); b.Property(x=>x.SellerProfitPercent).HasPrecision(9,4); b.Property(x=>x.Status).HasMaxLength(20).IsRequired(); b.Property(x=>x.RowVersion).IsRowVersion(); b.HasIndex(x=>x.Slug).IsUnique(); b.HasIndex(x=>x.Code).IsUnique(); b.HasOne(x=>x.ProductType).WithMany().HasForeignKey(x=>x.ProductTypeId).OnDelete(DeleteBehavior.Restrict); b.HasOne(x=>x.PrimaryCategory).WithMany().HasForeignKey(x=>x.PrimaryCategoryId).OnDelete(DeleteBehavior.SetNull); }
}
...
public sealed class ProductPieceConfiguration : IEntityTypeConfiguration<ProductPiece> { public void Configure(EntityTypeBuilder<ProductPiece>b){b.ToTable("ProductPiece",t=>{t.HasCheckConstraint("CK_ProductPiece_Weight","[ExactGoldWeightGrams] > 0");t.HasCheckConstraint("CK_ProductPiece_Quantity","[Quantity] >= 0");t.HasCheckConstraint("CK_ProductPiece_Status","[Status] IN (N'available',N'reserved',N'sold',N'damaged')");});b.HasKey(x=>x.Id);b.Property(x=>x.TrackingCode).HasMaxLength(100).IsRequired();b.Property(x=>x.ExactGoldWeightGrams).HasPrecision(10,3);b.Property(x=>x.Quantity).HasDefaultValue(1).ValueGeneratedNever();b.Property(x=>x.StoneWeightCarats).HasPrecision(10,3);b.Property(x=>x.Status).HasMaxLength(20).IsRequired();b.Property(x=>x.RowVersion).IsRowVersion();b.HasIndex(x=>x.TrackingCode).IsUnique();b.HasOne(x=>x.ProductVariant).WithMany(x=>x.Pieces).HasForeignKey(x=>x.ProductVariantId).OnDelete(DeleteBehavior.Cascade);} }

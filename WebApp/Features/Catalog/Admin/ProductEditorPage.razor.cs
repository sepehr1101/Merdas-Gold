...
{
    private static readonly (int Number, string Title, string Description)[] Steps = [(1, "هویت کالا", "نام و دسته‌بندی"), (2, "مشخصات", "ویژگی و محتوا"), (3, "تصاویر", "تصویر شاخص و گالری"), (4, "موجودی", "تنوع، وزن و وضعیت")];
    ...
    private string? Message => Error switch { "variant-invalid" => "اطلاعات تنوع را کامل کنید.", "piece-invalid" => "کد و وزن دقیق قطعه الزامی است.", "image-not-found" => "تصویر موردنظر پیدا نشد؛ ممکن است قبلاً حذف شده باشد.", "image-invalid" => "تصویر معتبر نیست؛ PNG، JPG یا WebP تا ۵ مگابایت انتخاب کنید.", "conflict" => "اطلاعات هم‌زمان تغییر کرده است؛ صفحه را بازخوانی کنید.", "invalid" => "اطلاعات تکراری یا نامعتبر است.", _ => Status switch { "saved" => "اطلاعات کالا ذخیره شد؛ حالا می‌توانید تصاویر آن را اضافه کنید.", "variant-saved" => "تنوع کالا با موفقیت ذخیره شد.", "piece-saved" => "قطعه وزن‌دار با موفقیت ذخیره شد.", "image-deleted" => "تصویر حذف شد.", "image-primary" => "تصویر شاخص محصول تغییر کرد.", "image-saved" => "تصویر کالا با موفقیت اضافه شد.", _ => null } };
    ...
    protected override async Task OnParametersSetAsync() { _loading = true; try { _data = await Service.GetEditorAsync(Id, TypeId); _step = Math.Clamp(RequestedStep ?? ((VariantId.HasValue || PieceId.HasValue) ? 4 : 1), 1, Id.HasValue ? 4 : 2); _variantEditor = VariantId.HasValue ? (VariantId == 0 ? new ProductVariantModel { Title = "", DisplayOrder = _data.Variants.Count + 1, IsActive = true } : _data.Variants.FirstOrDefault(x => x.Id == VariantId)) : null; _pieceVariantId = PieceVariantId; var variant = _data.Variants.FirstOrDefault(x => x.Id == PieceVariantId); _pieceEditor = PieceId.HasValue ? (PieceId == 0 ? new ProductPieceModel { Status = "available", IsActive = true } : variant?.Pieces.FirstOrDefault(x => x.Id == PieceId)) : null; } finally { _loading = false; } }
    private void GoToStep(int step) { if (!Id.HasValue && step > 2) return; _step = Math.Clamp(step, 1, Id.HasValue ? 4 : 2); }
    ...
    private bool IsStepComplete(int step) => step switch { 1 or 2 => Id.HasValue, 3 => _data?.Images.Count > 0, 4 => PieceCount > 0, _ => false };
